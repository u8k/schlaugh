...
var exposition = `<u>Objective:</u><br>Be the only player left with any units on the board<br><br><u>Mechanics:</u><br>You control the movement of your Units. You can schedule movements at any time during the day. All the action takes place at the schlaupdate. Your scheduled moves are secret until the schlaupdate.<br><br>Units live on Tiles. A tile containing at least one of your units is Occupied by you. The color of a tile indicates which player currently occupies it. The number on a tile indicates how many units occupy it. The maximum allowed units on a single tile is 17.<br><br><br><u>The Schlaupdate:</u><br>at the strike of Schlaupdate, the following occurs, in order:<br><ol><li>Migration</li><li>War</li><li>Creation</li><li>Entropy</li></ol><br><u>Migration</u><br>Every unit, on every turn, can either Move or Stay. For each tile, you may pick <i>one</i> of the tile's adjacent tiles to move units to, and can move some or all units, leaving the rest on their original tile.<br><br>By default, all units Stay. They only Move if you assign it.<br><br>If you move more than the maximum of 17 units to a tile, then at this time the extra units will be destroyed, leaving exactly 17.<br><br><u>War</u><br>Units on the same Tile as another player's Units will fight. Units on opposing sides destroy each other one-for-one. For example, if 7 red and 10 blue units occupy the same Tile, then after the battle the tile will be held by Blue with 3 remaining units. Ties result in all units destroyed and no one occupying the Tile.<br><br>If more than 2 players have units on a tile, then units from all players simultaneously destroy each other in the same manner. For example, if red has 6 units on a tile , and 5 other players also each have 5 units on the tile, then after battle only 1 red unit will remain.<br><br><u>Creation</u><br>If a tile is: <br><ol><li>unoccupied</li><li>adjacent to <i>exactly</i> 4(four) tiles that are occupied by the same player</li></ol>Then: 10 new units will spawn on the tile, belonging to the player occupying the four(4) adjacent tiles.<br><br><u>Entropy</u><br>units are subtracted from every occupied tile, depending on the tile's number of adjacent like-colored units, according to the following table:<br><l><ascii><u>neighbors    entropy</u><br>0            0<br>1            1<br>2            1<br>3            2<br>4            3<br>5            5<br>6            8<br></ascii></l><br><note linkText="">an alternate metaphor, which may or may not make the gameplay more or less easy to grok:<br>is to think of the units instead as health, belonging to a single unit<br>you spawn in a new unit that starts with 10 HP<br>units can fuse with each other to combine HP, up to a set maximum<br>a unit can un-fuse, splitting off part of itself to be a new unit living on an adjacent tile<br>(but can only perform one such split at a time)<br>units are decaying and lose HP to entropy each round<br>    the amount lost depending on their environment<br></note><br><hr>i am probably forgetting things, please ask questions. You can do so by messaging the <a href="/schlaunquer">schlaunquer account</a>, or tagging a post with <code>schlaunquer</code>`;
